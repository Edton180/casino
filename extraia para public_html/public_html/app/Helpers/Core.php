...
class Core
{
    ...
    public static function DiscountBalance(Wallet $wallet, $bet)
    {
        ...
        if ($wallet->balance >= $bet) {
            ...
            $wallet->increment('cashback', $bet * 0.10);
    ...
    /**Melhorado Por @vinixxxxxxxxxxxxxxxxxxxxx
    *ANTES DO UPDATE O SALDO NAO PODIA SER COMBINADO E BUGAVA,
    * EXEMPLO, voce tinha 30 centavos de saldo saque e ia apostar
    * mesmo tendo bonus nao descontava o saldo e ficava falando sem saldo
    */

            // Verifica se o saldo de saque e bônus combinados podem cobrir a aposta
            $totalAvailable = $wallet->balance_withdrawal + $wallet->balance_bonus;
            if ($totalAvailable >= $bet) {
                $withdrawalAmount = min($wallet->balance_withdrawal, $bet);
                $bonusAmount = min($wallet->balance_bonus, $bet - $withdrawalAmount);
                $wallet->decrement('balance_withdrawal', $withdrawalAmount);
                $wallet->decrement('balance_bonus', $bonusAmount);
                return 'balance_withdrawal_and_bonus';
            }
    ...
    public static function getDistribution(): array
    {
        return [
            ...
            "apipragmatic40" => "Center Gaming Pragmatic",
            "apipg12" => "Center Gaming PG",
