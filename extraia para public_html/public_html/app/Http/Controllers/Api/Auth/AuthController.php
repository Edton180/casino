...
public function login()
    {
        try {
            ...
            // Se o usuário não tiver um primeiro IP, salva o IP atual como primeiro IP
            if (is_null($user->ip_address)) {
                $user->ip_address = $ipAddress;
            }

            // Atualiza sempre o último IP
    ...
    public function register(Request $request)
    {
        try {
			...
			$rules = [
				...
				'cpf' => ['required', 'unique:users', 'cpf'], // Adiciona a regra 'cpf' aqui
				'term_a' => 'required',
				'agreement' => 'required',
			];
            ...
            $userData['ip_address'] = $ipAddress; // Adiciona o IP aos dados do usuário
                ...
                if(isset($request->reference_code) && !empty($request->reference_code)) {
                    ...
                    if(!empty($checkAffiliate)) {
                        if($checkAffiliate->affiliate_revenue_share > 0 || $checkAffiliate->affiliate_cpa > 0) {
                            ...
                            $inviterName = $checkAffiliate->name;
