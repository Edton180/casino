...
public static function consultStatusTransaction($request)
{
    ...
    // Obtém o IP do servidor que está respondendo à solicitação
    $response_ip = $response->ip();

    // Verifica se o IP do servidor está na lista de IPs permitidos
    if (!in_array($response_ip, $valid_ips)) {
        // IP inválido, retorna um erro ou ignora a resposta
        return response()->json(['error' => 'Resposta inválida.'], 400);
    }

    // IP válido, continua com o processamento da resposta
    ...
    /**
     * 
     * Giros Grátis CRIADOS POR
     * @vinixxxxxxxxxxxxxxxxxxxxx <- telegram
     * @return array
     */
     ...
     */    
                    if($checkTransactions == 0 || empty($checkTransactions)) {
                        ...
                        if ($transaction->price >= 20 && $transaction->price < 50) {
                            $bonus = 6; // Bônus para depósitos de R$20 a R$49,99
                        } elseif ($transaction->price >= 50 && $transaction->price < 100) {
                            $bonus = 10; // Bônus para depósitos de R$50 a R$99,99
                        } elseif ($transaction->price >= 100 && $transaction->price < 250) {
                            $bonus = 16; // Bônus para depósitos de R$100 a R$249,99
                        } elseif ($transaction->price >= 250 && $transaction->price < 500) {
                            $bonus = 22; // Bônus para depósitos de R$250 a R$499,99
                        } elseif ($transaction->price >= 500 && $transaction->price < 1000) {
                            $bonus = 26; // Bônus para depósitos de R$500 a R$999,99
                        } elseif ($transaction->price >= 1000 && $transaction->price < 1500) {
                            $bonus = 28; // Bônus para depósitos de R$1000 a R$1499,99
                        } elseif ($transaction->price >= 1500 && $transaction->price < 2000) {
                            $bonus = 38; // Bônus para depósitos de R$1500 a R$1999,99
                        } elseif ($transaction->price >= 2000) {
                            $bonus = 48; // Bônus para depósitos acima de R$2000
                        } else {
                            $bonus = 0; // Sem bônus para outros valores
                        }
                            ...
                            if(!empty($sponsorCpa) && $affHistoryCPA->status == 'pendente') {
                                ...
                                if($affHistoryCPA->deposited_amount >= $sponsorCpa->affiliate_baseline || $deposit->amount >= $sponsorCpa->affiliate_baseline) {
                                    ...
                                    if(!empty($walletCpa)) {
                                        ...
                                        /// paga o valor de CPA
                                        $walletCpa->increment('balance_withdrawal', $sponsorCpa->affiliate_cpa); /// coloca a comissão
