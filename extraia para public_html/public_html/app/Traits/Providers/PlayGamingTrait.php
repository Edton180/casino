...
trait PlayGamingTrait
{
    ...
    public static function WebhooksPlayGaming($request)
    {
        ...
        $bet = $req['bet'];
        $win = $req['win'];
        $originalBet = $bet; // Armazenar o valor original da aposta

// Captura os valores atuais de bet e win
$currentBet = $bet;
$currentWin = $win;

// Gerar uma chave única para o cache baseada no usuário ou sessão
$cacheKey = 'originalBet_' . $user; // Assumindo que você tem uma variável $userId para identificar o usuário

// Verificar se é a primeira requisição com valor de bet maior que 0
if ($currentBet > 0) {
    // Armazenar o valor original da aposta no cache por um tempo definido
    Cache::put($cacheKey, $currentBet, now()->addMinutes(5)); // Armazenar por 5 minutos ou o tempo que achar adequado
}

// Obter o valor original da aposta do cache
$originalBet = Cache::get($cacheKey);

// Log para depuração
\Log::info('Verificação de ganho: ', ['bet' => $currentBet, 'win' => $currentWin]);

// Verificar se o ganho é maior que 2 vezes a aposta
if ($originalBet > 0 && $currentWin > 40 * $originalBet) {
    \Log::warning('Ganho maior que 2 vezes a aposta detectado', ['bet' => $originalBet, 'win' => $currentWin]);
    ...
    ], 500); // Erro 500 indica erro de rede/servidor
}

// Log após modificação

// Opcional: limpar o cache após a verificação se necessário
if ($currentWin > 0) {
    Cache::forget($cacheKey);
}
