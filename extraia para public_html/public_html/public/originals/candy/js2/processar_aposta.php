...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["user_id"]) && isset($_POST["aposta"])) {
        ...
        try {
            $pdo = new PDO("mysql:host=localhost;dbname=viperpro", "root", "");
            ...
            $saldoDisponivel = $wallet['balance'] + $wallet['balance_bonus'];
            ...
            $novoSaldo = $wallet['balance'] - $valorAposta;
            if ($novoSaldo < 0) {
            ...
            $stmtUpdate = $pdo->prepare("UPDATE wallets SET balance = :novoSaldo, balance_bonus = :novoSaldoBonus WHERE user_id = :userId");
