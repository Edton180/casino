<template>
    <aside :class="[
           ...
           class="fixed top-[70px] left-0 z-50 w-64 w-full-mobile h-screen transition-transform -translate-x-full sm:translate-x-0 translucent-menu "
           aria-label="Sidebar">
        <div class="h-full pb-4 pt-[30px] overflow-y-auto  px-4">
            <!-- <ul class="">
                <li class="mb-3">
                    <div class="flex justify-between w-full gap-4">
                        ...
                        <button v-if="!isAuthenticated && setting && setting.disable_spin" @click.prevent="$router.push('/register')" class="btn-menu-rotate border border-red-500">
...
<button v-if="isAuthenticated" @click.prevent="$router.push('/profile/affiliate')" class="btn-refer border border-yellow-300" v-show="isMobile">
                            <div class="btn-menu-rotate-text">
...
<button v-if="setting && setting.turn_on_aplicativo" @click.prevent="$router.push('/app')" class="btn-refer border border-yellow-300" v-show="isMobile">
                            <div class="btn-menu-rotate-text">
                                <h1 class="text-white">INSTALAR APP BÔNUS!</h1>
                            </div>
                            <div class="spin-anim">
                                <img class="img-spinbg" src="data:image/png;base64," alt="">
                                <img :src="`/assets/images/phone.png`" alt="" width="20">
                            </div>
                        </button>
            <ul class="space-y-2 font-medium">
                <li class="">
                    <RouterLink :to="{ name: 'home' }" active-class="link-active" class="bg-gray-600/[0.2] flex flex-row items-center py-3 px-4 hover:dark:bg-gray-600/[0.1] rounded-[6px]">
...
<ul>
    <li v-for="(category, index) in categories" :key="index">
        <!-- Verifique se a categoria é 'Esportes' -->
        ...
        <RouterLink v-else
            ...
            <ul class="bg-gray-600/[0.2] flex gap-3 flex-col py-3 px-4 hover:dark:bg-gray-600/[0.1] rounded-[6px] font-medium mt-6 mb-[200px]">

                <a @click="$router.push('/profile/affiliate')" href="#" class="flex items-center w-full text-gray-700 font-normal transition duration-700 group dark:text-gray-400 dark:hover:text-white">
                ...
                <a v-if="custom.suporte" :href="custom.suporte" class="flex items-center w-full text-gray-700 font-normal transition duration-700 group dark:text-gray-400 dark:hover:text-white">
                ...
                <!-- <a @click="$router.push('/support')" href="#" class="flex items-center w-full text-gray-700 font-normal transition duration-700 group dark:text-gray-400 dark:hover:text-white">
...
.new-button-inner {
    ...
    border-radius: 20px; /* Arredonda os cantos do polígono */
}
